...
def main():
    ...
    X_raw, y_raw, subjects_raw, conditions_raw, groups_raw = load_dataset_npz(raw_dataset_path)
    X_feat, y_feat, subjects_feat, conditions_feat, groups_feat = load_dataset_npz(feature_dataset_path)
    ...
    if not np.array_equal(y_raw, y_feat):
        raise ValueError("Raw and feature labels do not match.")

    if not np.array_equal(subjects_raw, subjects_feat):
        raise ValueError("Raw and feature subjects do not match.")

    if not np.array_equal(conditions_raw, conditions_feat):
        raise ValueError("Raw and feature conditions do not match.")

    if groups_raw is None or groups_feat is None:
        raise ValueError("Both raw and feature datasets must contain groups.")

    if not np.array_equal(groups_raw, groups_feat):
        raise ValueError("Raw and feature groups do not match.")
    ...
    if X_feat.shape[1] != n_channels:
        raise ValueError(
            f"Channel mismatch: raw has {n_channels}, features have {X_feat.shape[1]}"
        )

    base_training_config = config["training"]
    base_model_config = config["model"]["hybrid_model"]
    ...
    if validation_method == "GROUPKFOLD":
        ...
        groupkfold_params, metadata, artifact_path = load_groupkfold_loso_aggregate(config, "hybrid")
    ...
    for fold_idx, split in enumerate(splits, start=1):
        ...
        X_raw_train_np = X_raw[train_mask]
        X_feat_train_np = X_feat[train_mask]
        y_train_np = y[train_mask]

        X_raw_val_np = X_raw[val_mask]
        X_feat_val_np = X_feat[val_mask]
        y_val_np = y[val_mask]

        X_raw_test_np = X_raw[test_mask]
        X_feat_test_np = X_feat[test_mask]
        y_test = y[test_mask]

        X_raw_train_np, X_raw_val_np, X_raw_test_np = standardize_raw_train_val_test(
            X_raw_train_np,
            X_raw_val_np,
            X_raw_test_np,
        )

        X_feat_train_np, X_feat_val_np, X_feat_test_np = standardize_features_train_val_test(
            X_feat_train_np,
            X_feat_val_np,
            X_feat_test_np,
        )

        X_raw_train = torch.tensor(X_raw_train_np, dtype=torch.float32).unsqueeze(1)
        X_raw_val = torch.tensor(X_raw_val_np, dtype=torch.float32).unsqueeze(1)
        X_raw_test = torch.tensor(X_raw_test_np, dtype=torch.float32).unsqueeze(1)

        X_feat_train = torch.tensor(X_feat_train_np, dtype=torch.float32)
        X_feat_val = torch.tensor(X_feat_val_np, dtype=torch.float32)
        X_feat_test = torch.tensor(X_feat_test_np, dtype=torch.float32)
        ...
        print("X_raw_val:", X_raw_val.shape)
        print("X_feat_val:", X_feat_val.shape)
        ...
        model = CNNHybrid(
            ...
            raw_temporal_filters=model_config.get("raw_temporal_filters", 16),
            raw_kernel_size=model_config.get("raw_kernel_size", 64),
            raw_pool_size=model_config.get("raw_pool_size", 8),

            feature_conv1_filters=model_config.get("feature_conv1_filters", 32),
            feature_conv2_filters=model_config.get("feature_conv2_filters", 64),
            feature_kernel_size_1=model_config.get("feature_kernel_size_1", 3),
            feature_kernel_size_2=model_config.get("feature_kernel_size_2", 3),

            raw_embedding_units=model_config.get("raw_embedding_units", 64),
            feature_embedding_units=model_config.get("feature_embedding_units", 64),
            fusion_hidden_units=model_config.get("fusion_hidden_units", 64),
        ...
        y_prob, y_pred = predict_hybrid_multiclass_model(
            model=model,
            X_raw=X_raw_test,
            X_feat=X_feat_test,
            device=device,
        ...
        write_hyperparameter_manifest(
            results_dir, "hybrid_cnn", validation_method, fold_hyperparameters, groupkfold_aggregate
        )

        if outputs_config.get("save_models", True):
            model_path = models_dir / f"hybrid_cnn_fold_{fold_idx}_subjects_{test_subject_label}.pt"

            save_model_checkpoint(
                model_path=model_path,
                model=model,
                model_name="CNNHybridRawFeatures",
                model_config=model_config,
                training_config=training_config,
                fold_idx=fold_idx,
                test_subject=test_subject_label,
                ...
                input_shape={
                    "raw": (1, n_channels, n_timepoints),
                    "features": (n_channels, n_features),
                },
                history=history,
                extra={
                    "n_channels": n_channels,
    ...
    print("\n" + "=" * 80)
    print(f"FINAL {validation_method} SUMMARY - HYBRID RAW + FEATURES CNN")
    print("=" * 80)
    ...
    if outputs_config.get("save_metrics", True):
        save_fold_metrics_csv(
            results_path=results_dir / f"hybrid_cnn_{validation_method.lower()}_metrics.csv",
            fold_rows=fold_rows,
        )
    ...
    plot_loso_performance_summary(
        ...
        title=f"{validation_method} cross-validation: Hybrid Raw + Features CNN",
        chance_level=0.5,
    )

    save_loso_summary_report(
        output_path=results_dir / f"hybrid_cnn_{validation_method.lower()}_summary.txt",
        analysis_name=f"Hybrid Raw + Features CNN: {analysis_type}",
