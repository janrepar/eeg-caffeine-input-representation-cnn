...
def main():
    ...
    base_model_config = config["model"]["features_2d_model"]
    ...
    if validation_method == "GROUPKFOLD":
        ...
        groupkfold_params, metadata, artifact_path = load_groupkfold_loso_aggregate(config, "features2d")
    ...
    for fold_idx, split in enumerate(splits, start=1):
        ...
        if validation_method == "LOSO":
            hpo_path = Path(config["outputs"]["output_dir"]) / "hyperparameter_optimization" / "features2d" / f"best_params_subject_{test_subject_label}.json"
        ...
        model = CNNFeatures2D(
            ...
            conv1_filters=model_config.get("conv1_filters", 16),
            conv2_filters=model_config.get("conv2_filters", 32),
            kernel_size_1=tuple(model_config.get("kernel_size_1", [3, 3])),
            kernel_size_2=tuple(model_config.get("kernel_size_2", [3, 3])),
            pool_size_1=tuple(model_config.get("pool_size_1", [2, 1])),
            pool_size_2=tuple(model_config.get("pool_size_2", [2, 1])),
            hidden_units=model_config.get("hidden_units", 64),
        )

        print_model_parameter_count(model, "Features2D CNN")
        ...
        write_hyperparameter_manifest(
            results_dir, "features2d_cnn", validation_method, fold_hyperparameters, groupkfold_aggregate
        )

        if outputs_config.get("save_models", True):
            model_path = models_dir / f"features2d_cnn_fold_{fold_idx}_subjects_{test_subject_label}.pt"

            save_model_checkpoint(
                model_path=model_path,
                model=model,
                model_name="CNNFeatures2D",
                model_config=model_config,
                training_config=training_config,
                fold_idx=fold_idx,
                test_subject=test_subject_label,
                val_subject=",".join(map(str, val_subjects)),
                input_shape=(1, n_channels, n_features),
                history=history,
                extra={
                    "n_channels": n_channels,
                    "n_features": n_features,
    ...
    print("\n" + "=" * 80)
    print(f"FINAL {validation_method} SUMMARY - FEATURES2D CNN")
    print("=" * 80)
    ...
    if outputs_config.get("save_metrics", True):
        save_fold_metrics_csv(
            results_path=results_dir / f"features2d_cnn_{validation_method.lower()}_metrics.csv",
            fold_rows=fold_rows,
        )
    ...
    plot_loso_performance_summary(
        ...
        title=f"{validation_method} cross-validation: Features2D CNN",
        chance_level=0.5,
    )

    save_loso_summary_report(
        output_path=results_dir / f"features2d_cnn_{validation_method.lower()}_summary.txt",
        analysis_name=f"Features2D CNN: {analysis_type}",
